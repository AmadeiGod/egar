...
public class CalendarPostServicesImpl implements CalendarPostServices {
    ...
    public void addPost(Menu form, @Valid ru.egartech.models.CalendarPost calendarPost, Authentication authentication, HttpServletRequest request) throws ParseException {
        ...
        if (formatter.parse(calendarPost.getDateDeleteString()).getTime() > date.getTime()) {
            try {
                if (calendarPostImpl.checkDishWarehouse(form)) {
                    calendarPost = calendarPostImpl.addCalendarPost(calendarPost, user, menu);
    ...
    public void calendarPostAddUser(Menu form, long id, Authentication authentication, HttpServletRequest request) throws ParseException {
        ...
        if (checkListForCheckUser) {
            if (checkListVisitUser) {
                try {
                    if (calendarPostImpl.checkMenuForDishAndCountDish(form, calendarPost)) {

                        for (int i = 0; i < form.getListDish().size(); i++) {
                            SendDish sendDish = dishServicesImpl.mapDishToSendDishForGuest(form.getListDish().get(i), user, calendarPost);
                            sendDishRepository.save(sendDish);
